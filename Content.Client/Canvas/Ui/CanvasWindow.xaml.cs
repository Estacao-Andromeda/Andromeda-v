...
namespace Content.Client.Canvas.Ui
{
    ...
    public sealed partial class CanvasWindow : FancyWindow
    {
        ...
        private string[] _paintingSegments = Array.Empty<string>();
        private bool _paintingSegmentsDirty = true;
        ...
        public void PopulatePaintingGrid()
        {
            ...
            for (int row = 0; row < _height; row++)
            {
                ...
                for (int col = 0; col < _width; col++)
                {
                    ...
                    Color initialColor = index < _paintingSegments.Length
                        ? GetColorFromCode(_paintingSegments[index])
                        : Color.White;
        ...
        private void UpdatePaintingCode(int row, int col, Color color)
        {
            ...
            if ((uint) index >= (uint) _paintingSegments.Length)
            {
                Logger.ErrorS("canvas", $"Index {index} is out of bounds for painting code.");
                return;
            }

            _paintingSegments[index] = $"{FormatColorComponent(color.R)}|{FormatColorComponent(color.G)}|{FormatColorComponent(color.B)}|{FormatColorComponent(color.A)}";
            _paintingCode = string.Join(";", _paintingSegments);
            OnSelected?.Invoke(_paintingCode);
        }
        ...
        {
            var trimmed = colorCode.AsSpan().Trim();
            if (TryParseColorCode(trimmed, out var color))
                return color;
        ...
        private static bool TryParseColorCode(ReadOnlySpan<char> colorCode, out Color color)
        {
            color = Color.White;

            var first = colorCode.IndexOf('|');
            if (first < 0)
                return false;

            var secondSpan = colorCode.Slice(first + 1);
            var secondRel = secondSpan.IndexOf('|');
            if (secondRel < 0)
                return false;
            var second = first + 1 + secondRel;

            var thirdSpan = colorCode.Slice(second + 1);
            var thirdRel = thirdSpan.IndexOf('|');
            if (thirdRel < 0)
                return false;
            var third = second + 1 + thirdRel;

            if (!TryParseColorComponent(colorCode.Slice(0, first), out var r)
                || !TryParseColorComponent(colorCode.Slice(first + 1, second - first - 1), out var g)
                || !TryParseColorComponent(colorCode.Slice(second + 1, third - second - 1), out var b)
                || !TryParseColorComponent(colorCode.Slice(third + 1), out var a))
            {
                return false;
            }
        ...
        {
            EnsurePaintingSegments();

            int requiredSegments = _width * _height;
            if (_paintingSegments.Length == requiredSegments)
                return;

            var newSegments = new string[requiredSegments];
            var copyCount = Math.Min(_paintingSegments.Length, requiredSegments);
            if (copyCount > 0)
                Array.Copy(_paintingSegments, newSegments, copyCount);

            var white = Color.White;
            var defaultColor = $"{FormatColorComponent(white.R)}|{FormatColorComponent(white.G)}|{FormatColorComponent(white.B)}|{FormatColorComponent(white.A)}";

            for (var i = copyCount; i < requiredSegments; i++)
            {
                newSegments[i] = defaultColor;
            }

            _paintingSegments = newSegments;
            _paintingCode = string.Join(";", _paintingSegments);
            _paintingSegmentsDirty = false;
        }

        private void EnsurePaintingSegments()
        {
            if (!_paintingSegmentsDirty)
                return;

            _paintingSegments = _paintingCode.Split(';', StringSplitOptions.RemoveEmptyEntries);
            _paintingSegmentsDirty = false;
        }
        ...
        private static bool TryParseColorComponent(ReadOnlySpan<char> component, out float value)
        {
            var componentString = component.ToString();
            return TryParseColorComponent(componentString, out value);
        }
